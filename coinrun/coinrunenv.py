...
from coinrun.common import mpi_util
...
game_versions = {"standard": 1000, "platform": 1001, "maze": 1002}
...
def init_args_and_threads(cpu_count=4, monitor_csv_policy="all"):
    ...
    dirmonitor = os.path.join(
        tempfile.gettempdir(),
        datetime.datetime.now().strftime("coinrunmonitor-%Y-%m-%d-%H-%M-%S-%f"),
    )
    dirmonitor = os.path.expanduser(dirmonitor)
    os.makedirs(os.path.expanduser(dirmonitor), exist_ok=True)

    lib.initialize_args(int_args)
    lib.initialize_set_monitor_dir(
        dirmonitor.encode("utf-8"),
...
class CoinRunVecEnv(VecEnv):
    ...
    def __init__(self, game_type, num_envs, lump_n=0, default_zoom=5.0):
        ...
        if self.hires_render:
            self.buf_render_rgb = np.zeros(
                [num_envs, self.VIDEORES, self.VIDEORES, 3], dtype=np.uint8
            )
        else:
        ...
        obs_space = gym.spaces.Box(
            0, 255, shape=[self.RES_H, self.RES_W, num_channels], dtype=np.uint8
        )
    ...
    def step_wait(self):
        ...
        lib.vec_wait(
            self.handle, self.buf_rgb, self.buf_render_rgb, self.buf_rew, self.buf_done
        )
...
def make(env_id, num_envs, **kwargs):
    assert env_id in game_versions, (
        'cannot find environment "%s", maybe you mean one of %s'
