...
class MpiAdamOptimizer(tf.Module):
    ...
    def _apply_gradients(self, avg_flat_grad, lr):
        ...
        a = (
            lr
            * tf.math.sqrt(1 - tf.math.pow(self.beta2, t))
            / (1 - tf.math.pow(self.beta1, t))
        )
        self.m.assign(self.beta1 * self.m + (1 - self.beta1) * avg_flat_grad)
        self.v.assign(
            self.beta2 * self.v + (1 - self.beta2) * tf.math.square(avg_flat_grad)
        )
...
def check_synced(localval, comm=None):
    ...
    if comm.rank == 0:
        assert all(
            val == vals[0] for val in vals[1:]
        ), "MpiAdamOptimizer detected that different workers have different weights: {}".format(
            vals
        )
