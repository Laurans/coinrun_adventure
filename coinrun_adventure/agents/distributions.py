...
class CategoricalPdType(PdType):
    def __init__(self, latent_shape, ncat, init_scale=1.0, init_bias=0.0):
        self.ncat = ncat
        self.matching_fc = _matching_fc(
            latent_shape, "pi", self.ncat, init_scale=init_scale, init_bias=init_bias
        )
...
class DiagGaussianPdType(PdType):
    def __init__(self, latent_shape, size, init_scale=1.0, init_bias=0.0):
        self.size = size
        self.matching_fc = _matching_fc(
            latent_shape, "pi", self.size, init_scale=init_scale, init_bias=init_bias
        )
        self.logstd = tf.Variable(
            np.zeros((1, self.size)), name="pi/logstd", dtype=tf.float32
        )
